  ...
  config =
    let
      ...
      cscli = pkgs.writeShellScriptBin "cscli" ''
        ${lib.getExe' cfg.package "cscli"} -c=${"${cfg.settings.general.config_paths.config_dir}/config.yaml"} "$@"
      '';
    ...
    lib.mkIf (cfg.enable) {
      ...
      systemd =
        let
          # merges the given service config (attrs) with the default service config set
          createServiceConfig =
            attrs:
            lib.mergeAttrs {
              User = cfg.user;
        ...
        in
        {
          ...
          # Reason: `environment.etc` is faster than `systemd.services.crowdsec.serviceConfig.ConfigurationDirectoryMode`
          # and creates the directory with 0755 mode
          tmpfiles.settings.crowdsec = {
            "${cfg.settings.general.config_paths.config_dir}".d = entry_permissions;
            "${cfg.settings.general.crowdsec_service.acquisition_dir}".d = entry_permissions;
          };
          ...
          services = {
            ...
            crowdsec = {
              ...
              serviceConfig = createServiceConfig {
                ...
                CapabilityBoundingSet = [
                  " " # Reset all capabilities to an empty set
                  "CAP_SYSLOG" # to be able to read syslog
                ];
                ExecReload = [
                ...
                ExecStart = [
                  ...
                  " "
                  "${lib.getExe' cfg.package "crowdsec"} -c ${"${cfg.settings.general.config_paths.config_dir}/config.yaml"} -info"
                ];
                ExecStartPre = [
                  " "
                  "${lib.getExe setupScript}"
                  "${lib.getExe' cfg.package "crowdsec"} -c ${"${cfg.settings.general.config_paths.config_dir}/config.yaml"} -t -error"
                ];
              };
            };
          };
